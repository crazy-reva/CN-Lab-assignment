...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"node_modules/mp4box/dist/mp4box.all.js":[function(require,module,exports) {
// file:src/log.js
/* 
...
},{}],"src/js/worker.js":[function(require,module,exports) {
var define;
"use strict";

var _mp4box = _interopRequireDefault(require("mp4box"));
...
function encqueue_report() {
  ...
  var len = encqueue_aggregate.all.length;
  var half = len >> 1;
  var f = len + 1 >> 2;
  var t = 3 * (len + 1) >> 2;
  var alpha1 = (len + 1) / 4 - Math.trunc((len + 1) / 4);
  var alpha3 = 3 * (len + 1) / 4 - Math.trunc(3 * (len + 1) / 4);
  var fquart = encqueue_aggregate.all[f] + alpha1 * (encqueue_aggregate.all[f + 1] - encqueue_aggregate.all[f]);
  var tquart = encqueue_aggregate.all[t] + alpha3 * (encqueue_aggregate.all[t + 1] - encqueue_aggregate.all[t]);
  var median = len % 2 === 1 ? encqueue_aggregate.all[len >> 1] : (encqueue_aggregate.all[half - 1] + encqueue_aggregate.all[half]) / 2;
  return {
    count: len,
...
function decqueue_report() {
  ...
  var len = decqueue_aggregate.all.length;
  var half = len >> 1;
  var f = len + 1 >> 2;
  var t = 3 * (len + 1) >> 2;
  var alpha1 = (len + 1) / 4 - Math.trunc((len + 1) / 4);
  var alpha3 = 3 * (len + 1) / 4 - Math.trunc(3 * (len + 1) / 4);
  var fquart = decqueue_aggregate.all[f] + alpha1 * (decqueue_aggregate.all[f + 1] - decqueue_aggregate.all[f]);
  var tquart = decqueue_aggregate.all[t] + alpha3 * (decqueue_aggregate.all[t + 1] - decqueue_aggregate.all[t]);
  var median = len % 2 === 1 ? decqueue_aggregate.all[len >> 1] : (decqueue_aggregate.all[half - 1] + decqueue_aggregate.all[half]) / 2;
  return {
    count: len,
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        ...
        case 18:
          ...
          self.postMessage({
            severity: "fatal",
            text: "Pipeline creation failed: ".concat(_context.t0.message)
          });
          return _context.abrupt("return");
        case 22:
        case "end":
...
var pipeline = /*#__PURE__*/function () {
  ...
  _createClass(pipeline, [{
    key: "DecodeVideoStream",
    value: function DecodeVideoStream(self) {
        ...
        start: function start(controller) {
          this.decoder = decoder = new VideoDecoder({
            ...
            error: function error(e) {
              self.postMessage({
                severity: "fatal",
                text: "Init Decoder error: ".concat(e.message)
              });
            }
          });
        },
        transform: function transform(chunk, controller) {
          ...
          if (this.decoder.state != "closed") {
            if (chunk.type == "config") {
              var config = JSON.parse(chunk.config);
              VideoDecoder.isConfigSupported(config).then(function (decoderSupport) {
                if (decoderSupport.supported) {
                  _this.decoder.configure(decoderSupport.config);
                  self.postMessage({
                    text: "Decoder successfully configured:\n" + JSON.stringify(decoderSupport.config)
                  });
                } else {
                  self.postMessage({
                    severity: "fatal",
                ...
                self.postMessage({
                  severity: "fatal",
                  text: "Configuration error: ".concat(e.message)
                });
              });
            } else {
              try {
                var queue = this.decoder.decodeQueueSize;
                decqueue_update(queue);
                this.decoder.decode(chunk);
              } catch (e) {
                self.postMessage({
                  severity: "fatal",
                  text: "Derror size: " + chunk.byteLength + " seq: " + chunk.seqNo + " kf: " + chunk.keyframeIndex + " delta: " + chunk.deltaframeIndex + " dur: " + chunk.duration + " ts: " + chunk.timestamp + " ssrc: " + chunk.ssrc + " pt: " + chunk.pt + " tid: " + chunk.temporalLayerId + " type: " + chunk.type
                });
                self.postMessage({
                  severity: "fatal",
                  text: "Catch Decode error: ".concat(e.message)
                });
              }
            }
          }
    ...
    key: "EncodeVideoStream",
    value: function EncodeVideoStream(self, config) {
      return new TransformStream({
        start: function start(controller) {
          var _this2 = this;
          this.frameCounter = 0;
          ...
          mp4boxfile.onSegment = function (id, user, buffer, sampleNum, isLast) {
            ...
            formData.append("segment", blob, "".concat(id, "_segment").concat(segmentNumber, ".mp4"));
          ...
          this.encoder = encoder = new VideoEncoder({
            output: function output(chunk, cfg) {
              ...
              if (cfg.decoderConfig) {
                ...
                self.postMessage({
                  text: "Configuration: " + decoderConfig
                });
                var configChunk = {
                  type: "config",
                  seqNo: _this2.seqNo,
                  keyframeIndex: _this2.keyframeIndex,
                  deltaframeIndex: _this2.deltaframeIndex,
                  timestamp: 0,
                  pt: 0,
              ...
              _this2.seqNo++;
              if (chunk.type == "key") {
                _this2.keyframeIndex++;
                _this2.deltaframeIndex = 0;
              } else {
                _this2.deltaframeIndex++;
              }
              _this2.pending_outputs--;
              chunk.seqNo = _this2.seqNo;
              chunk.keyframeIndex = _this2.keyframeIndex;
              chunk.deltaframeIndex = _this2.deltaframeIndex;
              controller.enqueue(chunk);
            },
            error: function error(e) {
              self.postMessage({
                severity: "fatal",
                text: "Encoder error: ".concat(e.message)
              });
            }
          });
        ...
        transform: function transform(frame, controller) {
          ...
          if (this.pending_outputs <= 30) {
            ...
            var insert_keyframe = this.frameCounter % config.keyInterval == 0;
            this.frameCounter++;
            try {
              if (this.encoder.state != "closed") {
                var queue = this.encoder.encodeQueueSize;
    ...
    key: "stop",
    value: function stop() {
      var encqueue_stats = encqueue_report();
      var decqueue_stats = decqueue_report();
      self.postMessage({
      ...
      self.postMessage({
        text: "stop(): frame, encoder and decoder closed"
      });
      return;
    }
  }, {
    ...
    value: function () {
      var _start = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2() {
        var duplexStream, readStream, writeStream;
        return _regeneratorRuntime().wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            ...
            case 2:
              started = true;
              self.postMessage({
                text: "Start method called."
              });
              _context2.prev = 4;
              _context2.next = 7;
              return this.inputStream.pipeThrough(this.EncodeVideoStream(self, this.config)).pipeThrough(this.DecodeVideoStream(self)).pipeTo(this.outputStream);
            case 7:
              _context2.next = 12;
              break;
            case 9:
              ...
              self.postMessage({
                severity: "fatal",
                text: "start error: ".concat(_context2.t0.message)
              });
            case 12:
            case "end":
...
},{"mp4box":"node_modules/mp4box/dist/mp4box.all.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/js/worker.js"], null)
//# sourceMappingURL=/worker.76c2667f.js.map